...
def quantize_channel(tensor, bits=8): #changed from bit=16 to bits=8
    ...
    for name, module in model.named_modules():
        ...
        if isinstance(module, quant_layers[0]) or isinstance(module, quant_layers[1]):
            ...
            for c in range(out_channels):
                ...
                dequant, error, scale = quantize_channel(original,bits=8) #Force bits to be 8
...
def parse_args():
    ...
    # resume
    parser.add_argument('--ckpt_path', default="home/riselabrtx1/Downloads/HAWQ_arun/hawq_arun/Pruning-and-Mixed-Precision-Quantization-of-Neural-Networks/checkpoints", type=str, help='checkpoint path to resume from')
    # run eval
...
if __name__ == '__main__':
    ...
    save_name = f'{args.identifier}.pth' if args.identifier else f'{args.arch}.pth'
    torch.save(net.state_dict(), os.path.join(model_path, save_name))
